<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
        ...
        <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==0" class="search-form round">
				...
				<input v-model="searchForm.techanmingcheng" type="text" placeholder="特产名称" ></input>
			</view>
			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.techanleixing" type="text" placeholder="特产类型" ></input>
			</view>
			<view class="action">
	...
	<scroll-view scroll-x="true" class="tabView" :style='{"padding":"8rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,1)","backgroundColor":"#e2e2e2","borderRadius":"0","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'>
        	<view :style='categoryName===item.techanleixing?sactiveItem:sitem' class="tab" v-for="(item,index) in categoryList" :key="index" :class="categoryName===item.techanleixing?'tabActive':''" @tap="categoryClick(item.techanleixing)">{{item.techanleixing}}</view>
        </scroll-view>
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"#efefef"}'>
			<view @tap="onDetailTap(product)" class="uni-product" :style='{"borderRadius":"8rpx","backgroundColor":"rgba(255, 255, 255, 1)"}' v-for="(product,index) in list" :key="index">
								                                <view class="uni-product-title" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"left"}'>{{product.techanmingcheng}}</view>
					...
					<text v-if="isAuth('techanxinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('techanxinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
				</view>
			</view>
		</view>
		
		
		<button :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,0) inset","backgroundColor":"#409EFF","borderColor":"#409EFF","borderRadius":"8rpx","color":"#fff","borderWidth":"2rpx","width":"160rpx","fontSize":"28rpx","borderStyle":"solid","height":"80rpx"}' v-if="isAuth('techanxinxi','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		async onShow() {
			...
			let res = await this.$api.list('techanleixing', {page:1,limit:100});
			res.data.list.splice(0,0,{id:0,techanleixing:'全部'})
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.techanmingcheng="";
				this.searchForm.techanleixing="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.techanleixing = '%' + this.categoryName + '%'
				}


				let res = await this.$api.list(`techanxinxi`, params);
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('techanxinxi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				let searchForm = {
					...
					limit: this.mescroll.size
												...
												if(this.searchForm.techanmingcheng){
					searchForm['techanmingcheng'] = '%' + this.searchForm.techanmingcheng + '%'
				}	
																if(this.searchForm.techanleixing){
					searchForm['techanleixing'] = '%' + this.searchForm.techanleixing + '%'
				}	
																																																																				let res = await this.$api.list(`techanxinxi`, searchForm);
				// 如果是第一页数据置空

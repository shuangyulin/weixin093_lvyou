	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
				...
				<view class="text icon" :style='{"color":"rgba(255, 0, 0, 1)","textAlign":"left","fontSize":"28rpx"}'><view v-if="storeupFlag==1" class="cuIcon-favorfill" @click="shoucang"></view><view v-if="storeupFlag==0" class="cuIcon-favor" @click="shoucang"></view></view>				
					...
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(6, 6, 6, 1)","textAlign":"left"}'>特产类型：</view>
					<view class="text" :style='{"color":"rgba(0, 0, 0, 1)","textAlign":"left","fontSize":"28rpx"}'>{{detail.techanleixing}}</view>
				</view>
                				                								                				<view class="detail-list-item" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0 0 2rpx 0","borderColor":"rgba(171, 174, 176, 1)","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none","height":"68rpx"}'>
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(6, 6, 6, 1)","textAlign":"left"}'>生产日期：</view>
					<view class="text" :style='{"color":"rgba(0, 0, 0, 1)","textAlign":"left","fontSize":"28rpx"}'>{{detail.shengchanriqi}}</view>
				</view>
                				                				<view class="detail-list-item" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0 0 2rpx 0","borderColor":"rgba(171, 174, 176, 1)","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none","height":"68rpx"}'>
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(6, 6, 6, 1)","textAlign":"left"}'>保质期：</view>
					<view class="text" :style='{"color":"rgba(0, 0, 0, 1)","textAlign":"left","fontSize":"28rpx"}'>{{detail.baozhiqi}}</view>
				</view>
				...
				<view class="content" :style='{"color":"#333","fontSize":"28rpx"}'>
					<rich-text style="font-size: 33upx;line-height: 50upx;letter-spacing: 5upx;" :nodes="detail.techanxiangqing"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','techanxinxi');
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('storeup', {
								userid: _this.user.id,
								name: _this.detail.techanmingcheng,
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','techanxinxi');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('techanxinxi', this.id);
				this.detail = res.data;
																												// 轮播图片
				this.swiperList = this.detail.tupian ? this.detail.tupian.split(",") : [];
																																//修改富文本的图片样式
				this.detail.techanxiangqing = this.detail.techanxiangqing.replace(/img src/gi,"img style=\"width:100%;\" src");
																																			},
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discusstechanxinxi', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
				...
				await this.$api.add('cart', {
					...
					goodid: this.detail.id,
															goodname: this.detail.techanmingcheng,
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusstechanxinxi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('techanxinxi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('techanxinxi', this.detail);
				this.$utils.msg('审核成功');

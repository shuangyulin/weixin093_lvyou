    ...
    >
      <el-row>
        ...
        <el-form-item class="select" v-if="type!='info'"  label="景点名称" prop="jingdianmingcheng">
          <el-select v-model="ruleForm.jingdianmingcheng" placeholder="请选择景点名称">
            <el-option
...
export default {
  ...
  methods: {
	...
	this.ruleForm.gonglvexiangqing = this.ruleForm.gonglvexiangqing.replace(reg,'../../../ssm4rlu0/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `jingdiangonglve/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jingdiangonglveCrossAddOrUpdateFlag = false;
                  this.parent.search();
